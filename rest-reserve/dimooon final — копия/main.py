...
def index():
    ...
    if request.method == "POST":
        ...
        return render_template('index.html', Title = 'Добро Пожаловать!', ur = 1, arr2 = arr2, arr = arr, head = head_rest, state1 = "Свободен", state2 = "Занят", var = 2, info = info, info2 = info, menu = hight_menu, tables = tabs, answer_true = "Занят", answer_false = "Свободен")
...
def user(username):
    ...
    if session['nameofuser'] == username: 
        ...
        if request.method == "POST":
            ...
            session["cht"] = tabs[choosed_table].id

            id = tabs[choosed_table].idr    # id status comment image
            status = tabs[choosed_table].status
            comment = tabs[choosed_table].comment
            image = tabs[choosed_table].image
            cost = tabs[choosed_table].cost
            
            

            return render_template('user.html',title = username, cost = cost, var1 = "Свободен", var2 = "Занят", tables = tabs, info = info, info2 = info, id = id, status = status, comment = comment, image = image, showtable = showtable)
    return render_template('user.html',title = username, tables = tabs, info = info, info2 = info)
